...
SQLitePlugin.prototype.readTransaction = async function (fn, error, success) {
...
SQLitePlugin.prototype.sqlBatch = function (sqlStatements) {
  if (!sqlStatements || sqlStatements.constructor !== Array) {
    throw newSQLError('sqlBatch expects an array');
  }
  const batchList = sqlStatements.map((st) => {
    if (st.constructor === Array) {
      if (st.length === 0) {
        throw newSQLError('sqlBatch array element of zero (0) length');
      }
      return { sql: st[0], params: st.slice(1) };
    }
    return { sql: st, params: [] };
  });

  return this.transaction((tx) =>
    Promise.all(batchList.map(({ sql, params }) => tx.executeSql(sql, params)))
  );
};
  ...
  if (this.dbname in this.openDBs) {
    if (txLocks[this.dbname] && txLocks[this.dbname].inProgress) {
      ...
      return Promise.reject(
        newSQLError('database cannot be closed while a transaction is in progress')
      );
    }
...
SQLitePlugin.prototype.attach = function (dbNameToAttach, dbAlias) {
  if (this.dbname in this.openDBs) {
    if (txLocks[this.dbname] && txLocks[this.dbname].inProgress) {
      ...
      return Promise.reject(
        newSQLError('database cannot be attached while a transaction is in progress')
      );
    }

    plugin.log(
      'ATTACH database ' + dbNameToAttach + ' to ' + this.dbname + ' with alias ' + dbAlias
    );

    return new Promise((resolve, reject) => {
      plugin.exec(
        'attach',
        { path: this.dbname, dbName: dbNameToAttach, dbAlias },
        resolve,
        reject
      );
    });
...
function transformParameters(values) {
  return !!values && values.constructor === Array
    ? values.map((v) => {
        ...
        if (t !== 'function') {
          plugin.warn(
            'addStatement - parameter of type <' + t + '> converted to string using toString()'
          );
          return v.toString();
        }
...
SQLitePluginTransaction.prototype.abort = async function (txFailure) {
  if (!this.started) {
    return Promise.reject({
      message: 'InvalidStateError: Transaction not started yet.',
    });
...
SQLiteFactory.prototype.openDatabase = argsArray(async function (args) {
  ...
  if (first.constructor === String) {
    ...
    if (
      !readOnly &&
      (!openargs.location ||
        openargs.location.constructor !== String ||
        !dblocations.hasOwnProperty(openargs.location))
    ) {
...
SQLiteFactory.prototype.echoTest = function () {
  return new Promise((resolve, reject) => {
    ...
    const success = function (testValue) {
      return testValue === inputTestValue
        ? resolve()
        : reject(`Mismatch: got: ${testValue} , expected: ${inputTestValue}`);
    };
...
SQLiteFactory.prototype.deleteDatabase = function (first) {
  ...
  if (first.constructor === String) {
    ...
    if (
      !first.location ||
      first.location.constructor !== String ||
      !dblocations.hasOwnProperty(first.location)
    ) {
      args.dblocation = dblocations['default'];
    } else {
